...
.PHONY: help config config-upgrade check install setup doctor detect-thread-boundaries dev dev-daemon start start-daemon stop up down clean docker-init docker-start docker-stop docker-logs docker-logs-frontend docker-logs-gateway build build-no-cache appgarden-build appgarden-build-no-cache build-registry kamiwaza-push kamiwaza-list kind-load-images appgarden-validate
